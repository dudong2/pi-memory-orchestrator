...
test("mentioning a sibling expands only that repository", () => {
  const plan = buildScopeQueryPlan(
    scope,
    "How did backend solve authentication?",
  );
  ...
  assert.ok(
    !plan.tags.includes("scope:repo:github.com/dudong2/infrastructure"),
  );
});
  ...
  const plan = buildScopeQueryPlan(
    {
      ...scope,
      knownRepositoryIds: [
        ...scope.knownRepositoryIds,
        "github.com/dudong2/auto_trading",
      ],
    },
...
test("workspace mode excludes every repository but keeps global knowledge", () => {
  const plan = buildScopeQueryPlan(scope, "shared conventions", {
    mode: "workspace",
  });
