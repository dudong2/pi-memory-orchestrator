...
const consolidation = await get(`/v1/default/banks/${bank}/operations/${migration.consolidationOperationId}`);
...
for (const { query, expectedHarness } of [
  ...
  const result = await get(`/v1/default/banks/${bank}/memories/list?q=${query}&limit=20`);
  const source = result.items?.find(
    (item: { metadata?: { harness?: string } }) => item.metadata?.harness === expectedHarness,
  );
  crossHarness.push({ query, count: result.total, expectedHarness, harness: source?.metadata?.harness });
}
const report = {
  version: 1,
  ...
  migration: {
    ...
    scopeRawFactsIncludingLiveWrites: migration.scopeRawFactsIncludingLiveWrites,
  ...
  wiring: {
    ...
    piHermesPackage: piSettings.packages?.includes("npm:pi-hermes-memory") ?? false,
    piOrchestratorPackage: piSettings.packages?.some((item: unknown) => String(item).includes("pi-memory-orchestrator")) ?? false,
    piOfficialHindsightExtension: piSettings.extensions?.some((item: unknown) => String(item).includes("hindsight/coding-agents")) ?? false,
    ompPiHermesEnabled: plugin("pi-hermes-memory")?.enabled === true,
    ompOfficialHindsightEnabled: plugin("@vectorize-io/hindsight-coding-agents")?.enabled === true,
    ompOrchestratorPackageEnabled: plugin("pi-memory-orchestrator")?.enabled === true,
    ompOrchestratorWrapper: existsSync(`${process.env.HOME}/.omp/agent/extensions/pi-memory-orchestrator.ts`),
    ompOfficialBridge: existsSync(`${process.env.HOME}/.omp/agent/extensions/hindsight-coding-agents.ts`) || ompConfig.includes("hindsight-coding-agents.ts"),
  },
  crossHarness,
  outbox,
  hindsight: {
...
const passed = report.acceptance.failedQueries === 0
...
await writeFile(join(backup, "cutover-report.json"), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
console.log(JSON.stringify({ passed, status: report.status, pendingConsolidation: report.hindsight.pendingConsolidation, observations: report.hindsight.observations }));
if (!passed) process.exitCode = 1;

async function get(path: string): Promise<any> {
  const response = await fetch(`${connection.apiUrl}${path}`, { headers, signal: AbortSignal.timeout(30_000) });
