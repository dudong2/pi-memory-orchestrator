...
const config = {
  ...
  recallTypes: ["observation", "world", "experience"] as Array<"observation" | "world" | "experience">,
...
const cases = [
  ...
  { name: "orchestrator", scope: scopes.resolved.orchestrator!, query: "How does the memory orchestrator use Hindsight tag_groups?", expected: ["tag_groups"] },
  { name: "memory", scope: scopes.resolved.memory!, query: "What Hindsight bank scoping rules were learned in the memory workspace?", expected: ["Hindsight", "bank"] },
];
const results = await Promise.all(cases.map(async (item) => {
  ...
  return {
    ...
    ok: !outcome.error && item.expected.every((token) => text.includes(token.toLowerCase())),
    error: outcome.error,
  };
}));
console.log(JSON.stringify(results.map((item) => ({ ...item, durationMs: Math.round(item.durationMs) }))));
if (results.some((item) => !item.ok || item.durationMs > 5_000)) process.exitCode = 1;
