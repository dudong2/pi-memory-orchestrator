...
if (!archivePath || !bankId)
  throw new Error(
    "usage: import-migration <archive.zip> <target-bank> [skip|replace|new-id]",
  );
if (conflict !== "skip" && conflict !== "replace" && conflict !== "new-id")
...
const importOperationId =
  typeof imported.operation_id === "string" ? imported.operation_id : undefined;
...
const consolidationOperationId =
  typeof consolidation.operation_id === "string"
    ? consolidation.operation_id
    : undefined;
