...
const manifest = parseJson<{
  ...
  await readFile(join(backup, "migrations", "final-import-batch.json"), "utf8"),
  "final import batch",
);
...
for (const item of manifest.results) {
  ...
  const response = await client.importDocuments(
    manifest.targetBank,
    archive,
    basename(item.outputArchive),
    "skip",
  );
  if (typeof response.operation_id === "string")
    await wait(response.operation_id, 300_000);
...
if (typeof consolidation.operation_id === "string")
  await wait(consolidation.operation_id, 900_000);
const result = {
  version: 1,
