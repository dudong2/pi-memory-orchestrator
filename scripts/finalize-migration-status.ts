...
if (!backup || !consolidationOperationId) throw new Error("usage: finalize-migration-status <backup-dir> <consolidation-operation-id>");
...
const missingDocuments = expected.filter((item) => !documentIds.has(item.documentId)).map((item) => `${item.bankId}/${item.documentId}`);
const scopeTags = [...new Set(expected.map((item) => item.scopeTag).filter((tag): tag is string => Boolean(tag)))];
const scopeCounts = [];
for (const tag of scopeTags) {
  const counts: Record<string, number> = {};
  for (const type of ["world", "experience", "observation"]) {
    const page = await get(`/v1/default/banks/${bank}/memories/list?type=${type}&tags=${encodeURIComponent(tag)}&tags_match=all_strict&limit=1000`) as { total: number };
    counts[type] = page.total;
  }
  scopeCounts.push({ tag, counts });
}
const tree = await get(`/v1/default/banks/${bank}/knowledge-base/tree`) as { roots?: Array<{ children?: unknown[] }> };
const operation = await get(`/v1/default/banks/${bank}/operations/${consolidationOperationId}`) as { status?: string };
const scopeRawFactCount = scopeCounts.reduce((sum, item) => sum + item.counts.world + item.counts.experience, 0);
const expectedFactCount = expected.reduce((sum, item) => sum + item.factCount, 0);
const migratedRawFactCount = expected.reduce(
  (sum, item) => sum + Number(documentMap.get(item.documentId)?.memory_unit_count ?? 0),
  0,
);
const result = {
  version: 1,
  ...
  migrationComplete: missingDocuments.length === 0 && expected.length === 30 && migratedRawFactCount === expectedFactCount,
};
if (!result.migrationComplete) throw new Error(`migration verification failed: ${JSON.stringify(result)}`);
await writeFile(join(backup, "migrations", "final-import-result.json"), `${JSON.stringify(result, null, 2)}\n`, { mode: 0o600 });
...
function countNodes(nodes: Array<{ children?: unknown[] }>): number {
  return nodes.reduce((sum, node) => sum + 1 + countNodes((node.children ?? []) as Array<{ children?: unknown[] }>), 0);
}
