...
if (command === "transform") {
  ...
  const documentIds = option("documents")
    ?.split(",")
    .map((item) => item.trim())
    .filter(Boolean);
  if (!sourceArchive || !outputArchive || !scopeTag || !documentIds?.length) {
    throw new Error(
      "transform requires --source, --output, --scope, and comma-separated --documents",
    );
  }
  ...
  await writeFile(
    manifestPath,
    `${JSON.stringify({ version: 1, createdAt: new Date().toISOString(), result, verification }, null, 2)}\n`,
    { mode: 0o600 },
  );
  process.stdout.write(
    `${JSON.stringify({ manifestPath, documentCount: verification.documentCount, factCount: verification.factCount })}\n`,
  );
} else if (command === "verify") {
  const archive = option("archive");
  const scopeTag = option("scope");
  if (!archive || !scopeTag)
    throw new Error("verify requires --archive and --scope");
  process.stdout.write(
    `${JSON.stringify(await verifyTransformedArchive(archive, scopeTag))}\n`,
  );
} else {
