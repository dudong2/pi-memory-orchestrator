...
export async function verifyTransformedArchive(
  ...
  try {
    ...
    const files = (await readdir(join(temporary, "documents"))).filter((name) =>
      name.endsWith(".json"),
    );
    const ids: string[] = [];
    let factCount = 0;
    for (const file of files) {
      const document = JSON.parse(
        await readFile(join(temporary, "documents", file), "utf8"),
      ) as {
        id: string;
        tags?: string[];
      ...
      if (JSON.stringify(document.tags) !== JSON.stringify([expectedScopeTag]))
      ...
      if (
        ...
        JSON.stringify(document.retain_params.observation_scopes) !==
          JSON.stringify([[expectedScopeTag]])
      ) {
        throw new Error(
      ...
      for (const fact of document.facts ?? []) {
        factCount++;
        if (JSON.stringify(fact.tags) !== JSON.stringify([expectedScopeTag]))
          throw new Error(`fact in ${document.id} has incorrect scope tags`);
        if (
          JSON.stringify(fact.observation_scopes) !==
          JSON.stringify([[expectedScopeTag]])
        )
          throw new Error(
            `fact in ${document.id} has incorrect observation scope`,
          );
        if (
          !fact.metadata?.migration_source_bank ||
          !fact.metadata.migration_source_document
        )
          throw new Error(`fact in ${document.id} lacks migration provenance`);
        if (fact.consolidated_at || fact.consolidation_failed_at)
          throw new Error(
            `fact in ${document.id} retains source consolidation lifecycle`,
          );
      }
    }
    if (
